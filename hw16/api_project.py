...
def main():
    jeonju_weather_2015 = './hw16/jeonju_weather_2015.csv'
    jeonju_weather_2022 = './hw16/jeonju_weather_2022.csv'
    jeonju_weather_2024 = './hw16/jeonju_weather_2024.csv'
    suwon_weather_2024 = './hw16/suwon_weather_2024.csv'
    if not os.path.exists(jeonju_weather_2015):
        download_weather(146, 2015, jeonju_weather_2015)
    if not os.path.exists(jeonju_weather_2022):
        download_weather(146, 2022, jeonju_weather_2022)
    if not os.path.exists(jeonju_weather_2024):
        download_weather(146, 2024, jeonju_weather_2024)
    if not os.path.exists(suwon_weather_2024):
        download_weather(119, 2024, suwon_weather_2024)

    # 1) 전주시(146)의 2015년 연 강수량은?
    rainfall_list_2015 =  get_weather_data(jeonju_weather_2015, 9)
    ...
    # 3) 전주시(146)의 2024년 최대 일교차(tmax-tmin)는?
    tmax_list_2024 = get_weather_data(jeonju_weather_2024,3)
    tmin_list_2024 = get_weather_data(jeonju_weather_2024,5)
    max_temp_diff_2024 = round(max_of_temp_diff(tmax_list_2024, tmin_list_2024),1)

    # 4) 수원시(119)와 전주시(146)의 2024년 총강수량 차이는(절댓값)?
    suwon_rainfall_list_2024 = get_weather_data(suwon_weather_2024,9)
    jeonju_rainfall_list_2024 = get_weather_data(jeonju_weather_2024,9)
    total_rainfall_diff_2024 = round(abs(sum(suwon_rainfall_list_2024) - sum(jeonju_rainfall_list_2024)),1)
    # print(total_rainfall_2015,max_of_tavg_2022,max_temp_diff_2024,total_rainfall_diff_2024)
